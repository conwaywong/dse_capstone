...
base_station_path= './data/{}/total_flow_{}_agg_transformed.pivot_{}_grouping.csv'
...
for year in years:
    for partition in partitions:
        #
        mean = pd.read_csv(base_mean_path.format(partition, partition, year), header=None).values[0]
        eigs = pd.read_csv(base_eigs_path.format(partition, partition, year), header=None).values.T  # eigenvectors per row matrix (5 X 288)
        stations_df = pd.read_csv(base_station_path.format(partition, partition, year), header=None)
        #
        stations_list= []
